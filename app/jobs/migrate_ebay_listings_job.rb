    ...
    listings_to_process.each_slice(BATCH_SIZE) do |batch|
      ...
      if kuralis_products_data.any?
        ...
        KuralisProduct.transaction do
          ...
          # Schedule image caching as a separate job to avoid blocking
          new_products.each do |product_data|
            CacheKuralisProductImagesJob.perform_later(shop.id, product_data["id"]) if product_data["image_urls"].present?
          end
